...
#include             "newstime.mqh"  // use this line if the file is in the same folder
...
int CNews::SaveHistory(bool printlog_info=false)
  {
   ...
   bool rt = CalendarValueHistory(eventvaluebuffer,last_update,tm_gmt);
   Print("Update status=",rt);
   ...
   for (int i=0;i<number_of_events;i++)
     {
      ...
      if (event[i].event_type!=CALENDAR_TYPE_HOLIDAY &&           // ignore holiday events
         event[i].timemode==CALENDAR_TIMEMODE_DATETIME)           // only events with exactly published time
        {
         ...
         FileWriteString(filehandle,eventbuffer.name,30);
         saved_elements++;
        }
     }
   // renew update time
