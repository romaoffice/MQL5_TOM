      ...
      if(magic==MAGICNUMBER && position_symbol==Symbol())
        {
            ...
            double bebuy = GlobalVariableGet(Symbol()+PERIOD_CURRENT+"lastworkbebuy"+MAGICNUMBER);
            double besell = GlobalVariableGet(Symbol()+PERIOD_CURRENT+"lastworkbesell"+MAGICNUMBER);
            ...
            if(positiveProfit){
               ...
               if(LOW2_after_timeout_value_Positive>0){//set for LOW2_after_timeout_value_Positive
                  ...
                  if(type== POSITION_TYPE_SELL && last_tick.bid<besell){//check touched be target
                     if((entry-last_tick.bid)/(entry-tp)>Breakeven_Touch_Ratio_After_Timeout_Positive){
